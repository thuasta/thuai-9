...
def run_match(match_id: int, agents: list[MatchAgent]) -> tuple[dict[int, int] | None, str]:
    ...
    try:
        ...
        for agent in agents:
            containers.append(_spawn_agent(client, agent))
            logger.info(
        ...
        while time.time() < deadline:
            ...
            if server is None:
                if latest_complete_scores is not None:
                    return _scores_by_submission(latest_complete_scores, token_to_submission), ""
            else:
                server.reload()
                if server.status in {"exited", "dead"}:
                    if latest_complete_scores is not None:
                        return _scores_by_submission(latest_complete_scores, token_to_submission), ""
                    return None, "游戏服已结束，但未写出完整结果"
